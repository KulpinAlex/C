...
long double s21_pow(double base, double exp) {
  long double res = 0.0;
  return (pow_check(base, exp, &res)) ? process_pow(base, exp) : res;
}

long double process_pow(double base, double exp) {
  long double res = 0.0;
  if ((long long int)exp == exp) {
    res = s21_powi(base, exp);
  } else {
    res = s21_powf(base, exp);
  }
  if (res > S21_DBL_MAX) {
    res = S21_INF;
  } else if (res < -S21_DBL_MAX) {
    res = -S21_INF;
  }
  return res;
}
...
int pow_check(double base, double exp, long double *res) {
